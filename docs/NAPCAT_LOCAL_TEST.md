...
QQ 入口使用独立的平台无关工具注册表，目前注册了 `roll_dice` 和 `dnd5e_check`。以后 COC、DND 等规则模块可以注册新的工具定义，不需要修改 OneBot 收发层或 Dice Engine。

## 8. 从 QQ 导入 D&D 2024（DND5r）XLSX 角色卡

先启动正式入口 `python -m src.qq_bot`。私聊中按下面的顺序操作最直观：

1. 把一个 `.xlsx` 角色卡文件发送给机器人。
2. 机器人提示收到后，在 5 分钟内发送 `.pc import`。
3. 机器人只会生成“导入草稿”和预览，不会立刻写成正式角色。
4. 检查预览中的角色名、职业、等级、属性、HP、AC，以及 ERROR / WARNING。
5. 没有需要处理的 ERROR 且你认可内容时，原样发送机器人给出的 `确认 <草稿ID>`。
...
群聊也可以上传，但必须由上传文件的同一个 QQ 号，在同一个群内于 5 分钟内发送 `.pc import`。这能避免甲上传的角色卡被乙误导入。群文件本身不需要 @机器人；这些传统命令也不会调用 LLM。

导入草稿和正式角色保存在 `data/trpg.sqlite3`，重启机器人后仍然存在。上传的 XLSX 只会进入临时目录，分析结束后即删除；不会自动保存到仓库，也不会提交到 Git。默认单文件上限是 10 MB，可在本机或 VPS 的 `.env` 中调整：

```dotenv
QQ_MAX_XLSX_MB=10
```

目前只支持已经能够确定性识别的 DND5r / D&D 2024 XLSX 模板。陌生模板会保留检查信息并给出错误或警告，但第一版还没有在 QQ 中逐字段纠正映射的对话界面。任何确认都必须由用户发送精确口令；LLM 无权替你确认或直接修改正式角色数据。
